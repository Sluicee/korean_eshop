                ...
                <table class="table table-sm">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><span class="edit_category">{{$item->name}}</span>
                                <div class="form-group edit_category cat_new_name_disabled">
                                    <input type="text" placeholder="{{$item->name}}" class="form-control cat_new_name">
                                </div>
                            </td>
                            
                            <td>{{$item->products->count()}}</td>
                            <td style="display: flex; gap: 10px">
                                <form method="GET" action="{{ route('admin.categoryDelete', $item->id) }}">
                                    @csrf
                                    <input name="_method" type="hidden" value="DELETE">
                                    <button type="submit" class="btn btn-xs btn-danger btn-flat show_confirm" data-toggle="tooltip" title='Delete'>Удалить</button>
                                </form>
                                ...
                                <form method="GET" action="{{ route('admin.categoryUpdate', $item->id) }}" class="edit_category cat_new_name_disabled">
                                    @csrf
                                    <input name="_method" type="hidden" value="EDIT">
                                    <input type="hidden" value="" name="new_cat_name" id="new_cat_name">
                                    <button type="submit" class="btn btn-xs btn-success btn-flat show_confirm" data-toggle="tooltip" title='Delete'>Подтеврдить</button>
                                </form>
                            </td>
                        </tr>
                        <script>
